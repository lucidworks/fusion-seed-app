...
angular.module('fusionSeed.viewDemoSearch', ['ngRoute','solr.Directives', 'demo.Directives', 'fusion.Directives'])
    .config(['$routeProvider', function($routeProvider) {
        $routeProvider.
            when('/demo/:category?', {
                templateUrl: 'demo/view-search.html',
                controller: 'ViewDemoSearchCtrl',
                reloadOnSearch: true
            });
    }])

    .controller('ViewDemoSearchCtrl', function ($scope, $http, $routeParams, $location, $route, $sce, $window, $interval, $timeout, fusionHttp, demoService) {


        $scope.loading = true;

        $scope.controller_path = demoService.controllerPath;

        $scope.noRecPipeline = demoService.pipelineNoRecId;
        $scope.defaultPipeline = demoService.pipelineId

        //pipeline_id and colleciton_id can be overriden by passing query params
        var pipeline_id = demoService.pipelineId;
        var collection_id = demoService.collectionId;
        ...
        var request_handler = demoService.requestHandler;
        var url = demoService.fusionUrl+'/api/apollo/query-pipelines/'+pipeline_id+'/collections/'+collection_id+'/'+request_handler;

        var collapse = undefined;
        if (demoService.collapseField)
            collapse = "{!collapse field="+demoService.collapseField+"}";


        $scope.clickCount = demoService.defaultSignalCount;
        ...
        var fqs = demoService.convertFqs(filter);
        ...
        if ($routeParams.cat) {
            ...
            if (cat.indexOf(demoService.taxonomySeparator) > -1) {
                var department = cat.split(demoService.taxonomySeparator)[0];
                var _class = cat.split(demoService.taxonomySeparator)[1];
        ...
        fusionHttp.getQueryPipeline(demoService.fusionUrl,pipeline_id,collection_id,request_handler,
            {
                'q': q,
                'fq': fqs,
                'start': start,
        ...
        function renderSearchResults(data, status, headers, config) {
            ...
            $scope.recBoosts = demoService.parseRecommendationBoost(solr_params.bq);


            var facet_fields = data.facet_counts.facet_fields;
            var facet_queries = data.facet_counts.facet_queries;
            var taxonomy = facet_fields[demoService.taxonomyField];
            ...
            if (demoService.taxonomyPivot) {
                $scope.taxonomy_pivot = data.facet_counts.facet_pivot[demoService.taxonomyPivot];
            }
            ...
            $scope.numPages = demoService.calculateNumPages(data.response.numFound,pageSize);
            //what page are we on?
            ...
            if (docCount == 0) {
                /*fusionHttp.getSpellCheck(demoService.fusionUrl,"ecomm_poc1-spellcheck",collection_id,q)
        ...
        $scope.sendSignal = function(signalType,docId,count,department,_class,manufacturer) {
            ...
            var filterHash = demoService.getFilterHash(filters);
            console.log(filterHash);


            //return demoService.sendSignal(signalType,docId,count,$routeParams.q,filterHash.department,filterHash.class,filterHash.manufacturer)
            return demoService.sendSignal(signalType,docId,count,$routeParams.q,department,_class,manufacturer)
        ...
        $scope.urlSafe = function(text) {
            return demoService.urlSafe(text);
        }
        ...
        $scope.updateAggrProgress = function() {
            ...
            if ($scope.totalAggrSecs > 4) {
                for (var i=0;i<demoService.aggrJobs.length;i++) {
                    var jobDone = false;
                    var url = demoService.fusionUrl+"/api/apollo/aggregator/jobs/products_signals/"+demoService.aggrJobs[i];
                    $http(
                        {method: 'GET',
                            url: url
                        })
        ...
        $scope.runAggregations = function() {
            ...
            var msg = "";
            for (var i=0;i<demoService.aggrJobs.length;i++) {
                fusionHttp.postRunAggr(demoService.fusionUrl,collection_id,demoService.aggrJobs[i])
                    .success(function(response) {
        ...
        $scope.typeAheadSearch = function(val) {
            return demoService.typeAheadSearch(val);
        };
        ...
        $scope.createCatLink = function(cat) {
            var q = "";
            if ($routeParams.q) q = $routeParams.q;
            return "#/"+demoService.controllerPath+"/"+encodePath(cat)+"?q="+q;
        }


        $scope.renderSearchResultsBullets = function(data) {
            return demoService.renderSearchResultsBullets(data);
        }
        ...
        $scope.clickPivot = function(department,_class) {
            ...
            if (_class) cat += demoService.taxonomySeparator + _class;
        ...
        $scope.renderHtml = function(html_code) {
            return demoService.renderHtml(html_code);
        };

        function encodePath(path) {
            return demoService.encodePath(path);
        };
