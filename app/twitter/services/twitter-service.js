var myModule = angular.module('fusionSeed.twitterService', []);

myModule.factory('twitterService', ['$http', 'fusionHttp', '$sce', function($http,fusionHttp,$sce) {
    ...
    var twitterService = {
        ...
        "pipelineId": "allyke-default", //the main pipeline that searches will go through
        "pipelineNoRecId": "allyke-default_copy", //the pipeline ID that searchs w/o recommendations should go through
        "simplePipelineId": "allyke-simple", // a 1 stage "Query Solr" pipeline with /select and /suggest enabled.
        "collectionId": "allyke", //the main collection
        "signalsCollectionId": "allyke_signals", //the signals colleciton
        "typeAheadCollectionId": "allyke_signals_aggr", //the collection where suggestions should come from
        ...
        convertFqs: function (filter) {
            ...
            if (twitterService.multiSelectFacets) {
                var new_fqs = [];
            ...
            if (twitterService.taxonomyField) new_fqs.push(cpath_fq);
        ...
        typeAheadSearch: function(val) {
            return fusionHttp.getQueryPipeline(twitterService.fusionUrl,twitterService.simplePipelineId,twitterService.typeAheadCollectionId,this.typeAheadReqHandler,
                {
                    wt: 'json',
